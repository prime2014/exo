...
def create_post(pk: int, post: dict):
    ...
    uid = post.get("uuid")
    params = {"pk": pk, "pub_date": post.get("pub_date"),
              "post": post.get("post"), "author": post.get("author"), "uuid": uid}
    query = '''MATCH (p1:Person {pgpk: $pk})
    CREATE (po:Post {uuid:$uuid, post:$post, author:$author, pub_date:$pub_date}),
    (po)-[:created_by]->(p1), (p1)-[:post]->(po) RETURN po'''
    try:
